...
`
	...
	testmessage := []llmstream.Message{
		{
			Role: "system",
			// Content: "You are a spaced repetition flashcard generator. Given source text, extract the most important concepts and generate flashcards. Rules: - Front: a clear, specific question or cloze prompt - Back: concise answer (1-3 sentences max) - Prefer atomic cards (one fact per card) - Output ONLY valid JSON, no markdown fences. Output format:{\"cards\": [{\"front\": \"...\", \"back\": \"...\"}]}",
			Content: prompt,
		},
		{
			Role:    "user",
	...
	_, err := llmstream.GenerateCard(client, &testmessage)
	if err != nil {
		log.Fatal(err)
		return
	}
